        ...
        >
          {mandateSteps.map((step) => (
            <motion.div
              key={step.number}
              variants={fadeInUp}
              className="group relative border border-outline-variant bg-white p-8 hover:border-secondary/50 hover:shadow-xl transition-all duration-500 overflow-hidden"
            >
              <div className="absolute inset-0 bg-gradient-to-br from-secondary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              <span className="font-serif text-6xl text-secondary/15 block mb-6 group-hover:text-secondary/30 transition-colors duration-300 leading-none">
                {step.number}
              </span>
          ...
          >
            {focusAreas.map((area) => (
              <motion.div
                key={area.label}
                variants={fadeInUp}
                className="flex items-center gap-4 p-5 border border-outline-variant group hover:border-secondary/50 hover:bg-white transition-all duration-300 cursor-default"
              >
                <span className="material-symbols-outlined text-secondary text-xl flex-shrink-0" aria-hidden="true">
                  {area.icon}
                </span>
